...
&nbsp;
## ℹ️ JSON-LD files
This folder contains the **JSON-LD serialisations** of all Arabic rhetorical devices and taxonomy items included in **Arabic Rhetorical Device Taxonomy (v0.1.1)**.

* Each file (`Qxx.jsonld`) corresponds to one Arabic rhetorical device or taxonomy item.  
* Each file (`Qxx.jsonld`) is exported directly from [**BalaghaBase.org**](https://balaghabase.org) (custom Wikibase instance).  
* JSON-LD is the *primary linked-data format* for web interoperability.  
* Files follow the **schema.org** and **Wikibase** JSON-LD contexts.  
* Ideal for: linked-data applications, graph ingestion, semantic web workflows.  
* Please see `/ttl/` and `/rdf/` for additional RDF-compatible formats.
